...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Freeze
    freeze = ['model.%s.' % x for x in range(int(opt.freeze_level)+1)]  # parameter names to freeze (full or partial)
    for k, v in model.named_parameters():
        v.requires_grad = True  # train all layers
        if any(x in k for x in freeze):
            print('freezing %s' % k)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--freeze_level', type=str, default=24, help='layer up to which we freeze the network')
